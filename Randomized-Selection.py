...
def randomized_select_with_multiple_pivots (current_array, k, no_of_pivots) :
    if len(current_array) == 1: return current_array[0]
    if no_of_pivots == 1:
        p = current_array[random.randint(0, len(current_array)-1)]
        
        lower = [x for x in current_array if x < p]
        equal = [x for x in current_array if x == p]
        higher = [x for x in current_array if x > p]
        n1 = len(lower)
        n2 = len(equal)
        
        if k < n1:
            return randomized_select_with_multiple_pivots(lower, k, 1)
        elif k > (n1+n2):
            return randomized_select_with_multiple_pivots(higher, k-n1-n2, 1)
        else:
            return p
    ...
    for i in range(no_of_pivots):
        p = current_array[random.randint(0, len(current_array)-1)]
        pivot.append(p)
        low.append([x for x in current_array if x <= p])
        high.append([x for x in current_array if x > p])
        
    n1 = [len(x) for x in low]
    n2 = [len(x) for x in high]
    
    check = []
    for i in range(no_of_pivots):
        check.append(-n2[i]) if n1[i] < k else check.append(n1[i])
    
    min_len = min(list(map(abs, check)))
    if min_len in check:
        ind = check.index(min_len)
        n = 0
        array = low[ind]
    ...
    return randomized_select_with_multiple_pivots(array, k-n, no_of_pivots-1)
...
for number_of_pivots in range(1, max_no_of_pivots+1) :
    ...
    for j in range(1, 40) :
        ...
        for i in range(1, number_of_trials) :
            t1 = time.time()
            answer1 = randomized_select_with_multiple_pivots(my_array,k,number_of_pivots)
            t2 = time.time()
